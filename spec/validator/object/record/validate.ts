...
import CallValidator from "../../../../dist/validator/object/record/validate";
import A from "@dikac/t-value/standard";
import Asserted from "@dikac/t-value/asserted";
import StdMessage from "@dikac/t-message/standard";
import Value from "@dikac/t-value/validatable/value";
import Mixin from "@dikac/t-value/validatable/mixin";
...
class ExtendedStr extends Str implements Validator<string, Validatable & Message<string> & Value<string>> {
    ...
    validate(value: string): Validatable & Message<string> & Value<string> {
        ...
        return new Asserted<string, Validatable & Value<string>>(
            new Mixin(
                new Standard(value),
                super.validate(value)
            ),
            new StdMessage('ExtendedNumAny')
        );
    }
}
...
class ExtendedNum extends Num implements Validator<number, Validatable & Message<string> & Value<number>> {
    ...
    validate(value: number): Validatable & Message<string> & Value<number>  {
        ...
        return new Asserted<number, Validatable & Value<number>>(
            new Mixin(
                new Standard(value),
                super.validate(value)
            ),
